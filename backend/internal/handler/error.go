...
var validationFieldLabels = map[string]string{
	...
	"Amount":           "Số tiền",
	"MinSpent":         "Mức chi tiêu tối thiểu",
...
func handleValidationError(c *gin.Context, err error) {
	...
	if errors.As(err, &perr) {
		response.BadRequest(c, fmt.Sprintf("Ngày giờ %q không đúng định dạng", perr.Value))
		return
	}
	// Nhánh cuối phải mang theo lý do thật. Trả về đúng một câu "Dữ liệu không
	// hợp lệ" cho MỌI lỗi giải mã nghĩa là người dùng — và cả người sửa lỗi —
	// không có cách nào biết trường nào sai; một ô ngày sinh gửi chuỗi rỗng
	// từng làm hỏng cả hộp thoại Thêm hội viên mà thông báo vẫn y hệt.
