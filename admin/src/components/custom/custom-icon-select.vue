...
<template>
  <ElPopover placement="bottom-end" trigger="click" width="334" :show-arrow="false">
    <div>
      <ElInput v-model="searchValue" :placeholder="$t('common.search')" />
    </div>
    ...
    <ElEmpty v-else class="w-306px" :description="$t('common.noData')" />
    <template #reference>
      <ElInput v-model:value="modelValue" readonly placeholder="Click to select">
        <template #suffix>
