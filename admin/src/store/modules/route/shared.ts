...
/**
 * Group flat VNET menu children into sub-groups (Management, Business, Operations, System)
 */
export function groupVnetMenus(menus: App.Global.Menu[]): App.Global.Menu[] {
  const vnetIndex = menus.findIndex(m => m.routeKey === 'vnet');
  if (vnetIndex === -1) return menus;

  const vnet = menus[vnetIndex];
  if (!vnet.children?.length) return menus;
  ...
  const children = vnet.children.map(c => ({
    ...c,
    icon: SvgIconVNode({ icon: iconMap[c.routeKey as string] || 'mdi:dots-square', fontSize: 20 }),
    i18nKey: c.i18nKey
  }));
  ...
  const groupedKeys = new Set(groups.flatMap(g => g.children));

  const topLevelMenus: App.Global.Menu[] = [];

  const dashboard = children.find(c => c.routeKey === 'vnet_dashboard');
  if (dashboard) {
    topLevelMenus.push(dashboard);
  }

  groups.forEach(group => {
    const groupChildren = group.children
      .map(k => children.find(c => c.routeKey === k))
      .filter(Boolean) as App.Global.Menu[];

    topLevelMenus.push({
      key: group.key,
      label: $t(group.i18nKey as any),
      i18nKey: group.i18nKey,
      routeKey: '' as any,
      routePath: (groupChildren[0]?.routePath || '') as any,
  ...
  menus.splice(vnetIndex, 1, ...topLevelMenus);
